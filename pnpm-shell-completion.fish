...
complete -c pnpm -n "not __fish_seen_subcommand_from remove" -a "(__get_string)"

complete -c pnpm  -f -l filter -r -a "$(FEATURE=filter pnpm-shell-completion)"
# add relative args
...
complete -c pnpm -n "__fish_seen_subcommand_from remove" -f -a "(__get_remove)"




function __get_string
